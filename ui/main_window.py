...
class MainWindow:
    ...
    def __init__(self, root, logger: DebugLogger):
        ...
        self.fractal_canvas.set_zoom_callback(
            self.on_zoom_confirm,
            self.on_zoom_cancel
        )

        self.logger.log(LogLevel.INIT, "フラクタルキャンバスの初期化開始")
        self.update_fractal()
    ...
    def update_fractal(self, *args):
        ...
        current_params = self.zoom_params.copy() # ズームパラメータをコピー
        current_params.update(panel_params) # パネル設定で上書き（max_iterなど）
        fractal_image = render_fractal(current_params, self.logger) # render_fractal が rotation キーを解釈できる前提
