...
from plugins import FractalTypeLoader, ColoringPluginLoader
...
# --- ColoringPluginLoaderクラスの定義 ---
# 通常は別のファイル (例: plugins/coloring_loader.py) に置くのが望ましいですが、
# ここでは main_window.py 内に定義します。
...
class MainWindow:
    ...
    def __init__(self, root: tk.Tk, logger: DebugLogger, config: dict):
        ...
        self.coloring_plugin_loader = ColoringPluginLoader(
            divergent_dir=divergent_plugin_dir,
            non_divergent_dir=non_divergent_plugin_dir,
            logger=self.logger
        )
        self.coloring_plugin_loader.scan_and_load_plugins() # ここで実際にプラグインを読み込む
        # --- カラーリングプラグイン読み込み完了 ---
    ...
    def _setup_parameter_frame(self) -> None:
        ...
        self.parameter_panel = ParameterPanel(
            ...
            logger=self.logger,
            config=self.config,
            fractal_loader=self.fractal_loader,
            coloring_plugin_loader=self.coloring_plugin_loader # カラーリングプラグインローダーを渡す
        )
    ...
    def _update_fractal_thread(self) -> None:
        ...
        try:
            ...
            # render_fractal に coloring_plugin_loader を渡す
            fractal_image = render_fractal(
                current_params,
                compute_function,
                self.logger,
                config=self.config,
                coloring_plugin_loader=self.coloring_plugin_loader # 追加
            )
