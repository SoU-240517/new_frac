import numpy as np
from typing import Dict, Tuple
from matplotlib.colors import Colormap

def apply_fourier_pattern(z: np.ndarray, iterations: np.ndarray, params: Dict, cmap: Colormap) -> np.ndarray:
    """フーリエ干渉パターン着色
    Args:
        z (np.ndarray): 複素数配列
        iterations (np.ndarray): 反復回数配列
        params (Dict): 着色パラメータ
        cmap (Colormap): 色マップ
    Returns:
        np.ndarray: 着色されたRGBA配列
    """
    non_divergent = iterations <= 0
    # フーリエ干渉パターンの計算
    with np.errstate(divide='ignore', invalid='ignore'):
        # 複素数の絶対値の対数を計算
        abs_log = np.log(np.abs(z[non_divergent]) + 1e-10)
        # 角度の計算
        angles = np.angle(z[non_divergent])
        # フーリエ干渉パターンの計算
        pattern = np.sin(abs_log * np.cos(angles * 3)) + np.cos(abs_log * np.sin(angles * 5))
        # 正規化
        normalized = (pattern - np.min(pattern)) / (np.max(pattern) - np.min(pattern))
        # ガンマ補正
        gamma = 1.3
        normalized = normalized ** (1/gamma)

    colored = np.zeros((*iterations.shape, 4), dtype=np.float32)
    colored[non_divergent] = cmap(normalized) * 255.0
    return colored
